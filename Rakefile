...
task :cfn_create_stack => [:cfn_generate] do
  run_cmds(["aws cloudformation create-stack --profile " + profile + " --stack-name " + cfn_stack_name + " --template-body file://" + project_dir + "/" + cfndsl_template + " --capabilities CAPABILITY_IAM",
            "aws cloudformation wait stack-create-complete --profile " + profile + " --stack-name " + cfn_stack_name])
end

task :cfn_update_stack => [:cfn_generate] do
  run_cmds(["aws cloudformation update-stack --profile " + profile + "  --stack-name " + cfn_stack_name + " --template-body file://" + project_dir + "/" + cfndsl_template + " --capabilities CAPABILITY_IAM",
            "aws cloudformation wait stack-update-complete --profile " + profile + " --stack-name " + cfn_stack_name])
end

task :delete_cfn_stack do
  run_cmds(["aws cloudformation delete-stack --profile " + profile + "   --stack-name " + cfn_stack_name,
            "aws cloudformation wait stack-delete-complete --profile " + profile + "  --stack-name " + cfn_stack_name])
end

task :display_cfn_stack do
  run_cmds(["aws cloudformation describe-stacks --profile " + profile + "  --stack-name " + cfn_stack_name])
end

task :lambda_update_running_code => [:lambda_create_code_zipfile] do
  run_cmds(["aws lambda update-function-code --profile " + profile + "  --function-name " +  lambda_function_name + " --zip fileb://" + lambda_code_zip])
end

task :lambda_update_s3_code => [:lambda_create_code_zipfile] do
  run_cmds(["aws s3 cp " + lambda_code_zip + " " + lambda_css_s3_bucket] +" --storage-class REDUCED_REDUNDANCY --profile " + profile)
end

task :lambda_update_css do
  run_cmds(["aws s3 cp main.css " + lambda_css_s3_bucket +" --storage-class REDUCED_REDUNDANCY --grants read=uri=http://acs.amazonaws.com/groups/global/AllUsers --profile " + profile])
end
